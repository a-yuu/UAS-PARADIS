...
# TEST 8: Perbandingan Publish Batch vs Tunggal
@pytest.mark.asyncio
...
# TEST 9: Validasi Payload (Integritas Data)
@pytest.mark.asyncio
async def test_9_payload_integrity(test_app):
    """Pastikan payload JSON yang dikirim tidak berubah saat diambil kembali"""
    agg, _ = test_app
    await agg.start()

    complex_payload = {"user_id": 123, "action": "login", "meta": {"ip": "127.0.0.1"}}
    e = Event(topic="integrity", event_id="evt_integrity_001",
               timestamp="2025-10-18T10:00:00Z", source="test", payload=complex_payload)
    
    await agg.publish([e])
    await asyncio.sleep(0.5)

    events = await agg.get_events("integrity")
    assert len(events) == 1
    assert events[0]["payload"] == complex_payload
    await agg.stop()

# TEST 10: Handling Empty/Large Batch
@pytest.mark.asyncio
async def test_10_empty_and_large_batch(test_app):
    """Uji ketahanan aggregator terhadap batch kosong dan batch cukup besar"""
    agg, _ = test_app
    await agg.start()

    # Kasus 1: Batch Kosong
    await agg.publish([])
    
    # Kasus 2: Batch 1000 event (Stress test kecil)
    large_batch = [Event(topic="large", event_id=f"large_{i}",
                         timestamp="2025-10-18T10:00:00Z", source="test", payload={})
                   for i in range(1000)]
    await agg.publish(large_batch)
    
    # Tunggu processing selesai
    while agg.queue.qsize() > 0:
        await asyncio.sleep(0.1)
    await asyncio.sleep(0.5)

    assert agg.stats["unique_processed"] == 1000
    await agg.stop()

# TEST 11: Deduplication across different Topics
@pytest.mark.asyncio
async def test_11_dedup_cross_topic(test_app):
    """Pastikan event_id yang sama di topik berbeda tetap dianggap duplikat (Global ID)"""
    agg, _ = test_app
    await agg.start()

    # ID sama, tapi topik berbeda
    e1 = Event(topic="topic.alpha", event_id="global_id_001",
               timestamp="2025-10-18T10:00:00Z", source="test", payload={})
    e2 = Event(topic="topic.beta", event_id="global_id_001",
               timestamp="2025-10-18T10:00:00Z", source="test", payload={})

    await agg.publish([e1])
    await asyncio.sleep(0.2)
    await agg.publish([e2])
    await asyncio.sleep(0.5)
...
# TEST 12: Clear Database Operation
@pytest.mark.asyncio
async def test_12_clear_store_functionality(test_app):
    """Pastikan fungsi clear menghapus semua data agar sistem bisa mulai dari nol"""
    agg, store = test_app
    await agg.start()

    e = Event(topic="clear", event_id="evt_clear",
               timestamp="2025-10-18T10:00:00Z", source="test", payload={})
    ...
    # Kirim event yang sama lagi -> harusnya dianggap unik lagi karena DB sudah bersih
    # Catatan: Kita reset stats manual untuk simulasi sistem fresh
    agg.stats = {"received": 0, "unique_processed": 0, "duplicate_dropped": 0}
    await agg.publish([e])
    await asyncio.sleep(0.5)
